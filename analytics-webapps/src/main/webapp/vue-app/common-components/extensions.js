...
extensionRegistry.registerExtension('AnalyticsSamples', 'SampleItem', {
  ...
  options: {
    ...
    match: fieldName => (fieldName === 'userId' || fieldName === 'modifierSocialId' || fieldName === 'identityId' || fieldName === 'identityId_alt'),
    options: {
      isUser: true,
    },
  },
...
extensionRegistry.registerExtension('AnalyticsSamples', 'SampleItem', {
  ...
  options: {
    rank: 70,
    vueComponent: Vue.options.components['analytics-profile-property-item-attribute'],
    includeLabel: () => true,
    match: (fieldName) => {
      return fieldName.startsWith('profileProperties.');
    }
  },
});
